...
document.addEventListener('DOMContentLoaded', () => {
  const start = async() => {
    mockWithVideo('./content/mockmock.mp4');
    ...
    const mindarThree = new window.MINDAR.IMAGE.MindARThree({
      ...
      filterBeta: 0.0001, 
      uiLoading: '#loading', 
      imageTargetSrc: './content/photo.mind',
    });
    ...
    const logoMask = await loadTexture("./content/logo.png");
    const logoMat = new THREE.MeshBasicMaterial({ color: 0xFFFFFF, alphaMap: logoMask, transparent:true});
    const logo = new THREE.Mesh(logoGeo, logoMat);
    logo.scale.set(0.15,0.15,0.15);
    logo.position.set(0,-(height/1.8),0.25);

    // VIDEO PLANE
    const video = await loadVideo("./content/video.mp4")
